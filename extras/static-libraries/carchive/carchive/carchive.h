...
public:
	...
	void *ReadFile(char *szFilename, unsigned long *retFilesize);

	int ReadFile(char *szFilename, BYTE *pFilebuff, DWORD *retFilesize);

	//return filesize
	unsigned long SizeFile(char *szFilename);

	//return compressed filesize in archive file
	unsigned long CompressedSizeFile(char *szFilename);	
