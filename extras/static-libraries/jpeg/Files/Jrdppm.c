...
/*
 * On most systems, reading individual bytes with jgetc() is drastically less
 ...
 * need to process very wide images on a PC, you may have to use the jgetc()
...
/* This should not be a problem in practice. */
{
  ...
  do {
    ch = pbm_getc(cinfo->input_filebuffer);
    if (ch == EOF)
  ...
  val = ch - '0';
  while ((ch = pbm_getc(cinfo->input_filebuffer)) >= '0' && ch <= '9') {
    val *= 10;
    val += ch - '0';
  }
  return val;
...
{
  register char * infile = cinfo->input_filebuffer;
  register JSAMPROW ptr0;
  register long col;
  
  ptr0 = pixel_row[0];
  for (col = cinfo->image_width; col > 0; col--) {
    *ptr0++ = rescale[jgetc(infile)];
  }
}
...
/* This version is for reading raw-format PPM files with any maxval */
{
  ...
  for (col = cinfo->image_width; col > 0; col--) {
    *ptr0++ = rescale[jgetc(infile)];
    *ptr1++ = rescale[jgetc(infile)];
    *ptr2++ = rescale[jgetc(infile)];
  }
}
...
/* This version is for reading raw-format PPM files with maxval = MAXJSAMPLE */
{
  ...
  for (col = cinfo->image_width; col > 0; col--) {
    *ptr0++ = (JSAMPLE) jgetc(infile);
    *ptr1++ = (JSAMPLE) jgetc(infile);
    *ptr2++ = (JSAMPLE) jgetc(infile);
  }
}
...
/* This version is for reading raw-format PGM files with any maxval */
{
  ...
  if (! ReadOK(cinfo->input_filebuffer, row_buffer, cinfo->image_width))
    ERREXIT(cinfo->emethods, "");
  ptr0 = pixel_row[0];
...
/* This version is for reading raw-format PPM files with any maxval */
{
  ...
  if (! ReadOK(cinfo->input_filebuffer, row_buffer, 3 * cinfo->image_width))
    ERREXIT(cinfo->emethods, "");
  ptr0 = pixel_row[0];
  ptr1 = pixel_row[1];
  ptr2 = pixel_row[2];
...
input_init (compress_info_ptr cinfo)
{
  ...
  if (jgetc(cinfo->input_filebuffer) != 'P')
    ERREXIT(cinfo->emethods, "");

  c = jgetc(cinfo->input_filebuffer);	/* save format discriminator for a sec */
