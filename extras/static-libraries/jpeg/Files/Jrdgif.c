...
/* A zero-length block marks the end of a data block sequence */
{
  ...
  if (count > 0) {
    if (! ReadOK(cinfo->input_filebuffer, buf, count))
      ERREXIT(cinfo->emethods, "");
  }
  return count;
}
...
input_init (compress_info_ptr cinfo)
{
  ...
  if (! ReadOK(cinfo->input_filebuffer, hdrbuf, 6))
    ERREXIT(cinfo->emethods, "");
  ...
  if (! ReadOK(cinfo->input_filebuffer, hdrbuf, 7))
    ERREXIT(cinfo->emethods, "");
  ...
  for (;;) {
    ...
    if (! ReadOK(cinfo->input_filebuffer, hdrbuf, 9))
      ERREXIT(cinfo->emethods, "");
