 ...
 */
	...
	FILE *  input_file;	/* tells input routines where to read image */
	FILE *  output_file;	/* tells output routines where to write JPEG */
	char *input_filebuffer;
	char *output_image;
	char *output_imageptr;
	...
	_boolean interleave;	/* TRUE=interleaved output, FALSE=not */
 ...
 */
	...
	FILE *  input_file;	/* tells input routines where to read JPEG */
	FILE *  output_file;	/* tells output routines where to write image */
	char *input_filebuffer;
	char *output_image;
	char *output_imageptr;
	...
	_boolean quantize_colors; /* T if output is a colormapped format */
	/* the following are ignored if not quantize_colors: */
	_boolean two_pass_quantize;	/* use two-pass color quantization? */
	_boolean use_dithering;		/* want color dithering? */
	int desired_number_of_colors;	/* max number of colors to use */

	_boolean do_block_smoothing; /* T = apply cross-block smoothing */
	_boolean do_pixel_smoothing; /* T = apply post-upsampling smoothing */
...
struct External_methods_struct {
	...
	METHOD(void *, alloc_medium, (size_t sizeofobject));
	METHOD(void, free_medium, (void *ptr));
...
struct Decompress_methods_struct {
	...
	METHOD(_boolean, read_scan_header, (decompress_info_ptr cinfo));
...
EXTERN void jcopy_block_row PP((JBLOCKROW input_row, JBLOCKROW output_row,
				long num_blocks));
EXTERN void jzero_far PP((void * target, size_t bytestozero));
